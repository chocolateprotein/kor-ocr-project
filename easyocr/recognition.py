...
def get_recognizer(recog_network, network_params, character,\
    ...
    if recog_network == 'generation1':
        ...
        module_path = 'user_network_directory.' + recog_network
        model_pkg = importlib.import_module(module_path)
 
    model = model_pkg.Model(num_class=num_class, **network_params )
    
    
    print('실행됐어!')
    # model = model_pkg.Model(input_channel = network_params['input_channel'], output_channel=network_params['output_channel'], hidden_size=network_params['hidden_size'], num_class=num_class)
    # # self, input_channel, output_channel, hidden_size, num_class
    # import os
    ...
    if device == 'cpu':
        ...
        # model.on_load_checkpoint(new_state_dict)
        if quantize:
            try:
